...
export default {
  ...
  setup() {
    ...
    this.mobile.scale.set(zoom.min * 2)
      ...
      tween({
        ...
        complete: () => {
          ...
          this.fill(indices).then(() => {
            ...
            indices.length ? resolve(this.clean(indices)) : resolve()
          })
        }
      })
    })
  ...
  listen() {
    ...
    this.layout.on('pointerdown', async ev => {
      ...
      if (!(target instanceof Cell) || !this.swappable) return
      if (!this.last) {
        this.last = target
        target.outline()
        return
        ...
        const indices = this.check(this.layout.children.map(child => child.id))
        indices.length ? await this.clean(indices) : await this.swap(target, this.last)
        this.last =
        target.filters =
